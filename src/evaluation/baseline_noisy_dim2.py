"""LR + MLP baseline on the noisy nearest-boundary dataset."""
import json
import time
...
def main():
    ...
    print("\n" + "=" * 60, flush=True)
    print(f"NOISY BASELINE DIM 2 ({N_SEEDS} seeds)", flush=True)
    print("=" * 60, flush=True)
    ...
    print(f"\nComparison with the other dim 2 regimes:", flush=True)
    print(f"  Easy (original):       LR=50.11%  MLP=95.37%", flush=True)
    print(f"  Boundary (structural): LR=51.02%  MLP=83.67%", flush=True)
    print(f"  Random (control):      LR=51.64%  MLP=99.26%", flush=True)
    print(f"  Noisy (new):     LR={np.mean(accs_lr):.2f}%  MLP={np.mean(accs_mlp):.2f}%", flush=True)

    with open("baseline_noisy_dim2.json", "w") as f:
        json.dump({
            "dataset": DATASET,
            "n_seeds": N_SEEDS,
    ...
    print("Saved: baseline_noisy_dim2.json", flush=True)
