"""Retrain the dedicated dim=2 model over N seeds, using the same
architecture and hyperparameters as the model reported in the paper. At
the end of each seed the model is evaluated on
`dataset_noisy_topologico_dim2_enriquecido.pt`.

Output: `semillas_10_dim2.json` with in-distribution test accuracy and
dim=2 noisy accuracy per seed, plus mean and standard deviation
across the N seeds.

Required files in the same directory:
  - dataset_enriquecido_dim2_50000_DATOS_dificiles.pt
  - dataset_noisy_topologico_dim2_enriquecido.pt

Usage:
    python -u 03_dim2_10seeds.py --n_seeds 10 --n_epocas 100
"""
import argparse
import json
import os
...
DATASET_TRAIN = "dataset_enriquecido_dim2_50000_DATOS_dificiles.pt"
DATASET_NOISY = "dataset_noisy_topologico_dim2_enriquecido.pt"
# Dedicated dim=2 model hyperparameters
LR = 0.0009717775305059633
DIM_OCULTA = 64
BATCH_SIZE = 16
...
class RedNovikov(nn.Module):
    def __init__(self, dim_nodos=6, dim_aristas=2, dim_oculta=64,
...
def entrenar_una_semilla(dataset, dispositivo, n_epocas, semilla,
    ...
    """Train one seed: 70/15/15 split, fixed seeds, best checkpoint by
    val_loss, test evaluated with that best checkpoint."""
    ...
    modelo = RedNovikov(dim_nodos=6, dim_aristas=2,
    ...
    with torch.no_grad():
        for lote in test_loader:
            ...
            total += etiq.size(0)
    return modelo, correctos / total * 100, mejor_val_loss, mejor_epoca
...
def main():
    ...
    parser.add_argument("--n_epocas", type=int, default=100)
    parser.add_argument("--output", default="semillas_10_dim2.json")
    ...
    for s in range(args.n_seeds):
        ...
        print(f"  noisy dim2 acc:     {eval_noisy['accuracy']:.2f}% "
    ...
    if resultados:
        ...
        noisy = np.array([r["noisy_dim2"]["accuracy"] for r in resultados])
        resumen = {
            "in_dist_mean": float(indist.mean()),
            "in_dist_std": float(indist.std(ddof=0)),
            "noisy_dim2_mean": float(noisy.mean()),
            "noisy_dim2_std": float(noisy.std(ddof=0)),
        ...
        print(f"  in-dist: {resumen['in_dist_mean']:.2f} +/- {resumen['in_dist_std']:.2f}",
              flush=True)
        print(f"  noisy d2:  {resumen['noisy_dim2_mean']:.2f} +/- {resumen['noisy_dim2_std']:.2f}",
              flush=True)
        with open(args.output, "w") as f:
            json.dump({"hp": {"lr": LR, "dim_oculta": DIM_OCULTA,
                              "batch_size": BATCH_SIZE},
